...
async def test_under_limit_passes():
    ...
    app = FastAPI()
    limiter = await get_rate_limiter(max_requests=5, window_seconds=60, force_in_memory=True)
...
async def test_over_limit_blocked():
    ...
    app = FastAPI()
    limiter = await get_rate_limiter(max_requests=3, window_seconds=60, force_in_memory=True)
