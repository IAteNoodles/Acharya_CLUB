...
| Admin dashboard mixed role and status axes | `UserStats` exposed `by_role` and `by_status` only, so role+status cards had no correct field: "Teacher accounts" used `by_status.pending` and the Students card was hinted with `by_status.active` | `backend/app/services/reports.py`, `frontend/src/pages/admin/DashboardPage.tsx` | ✅ Fixed — a seeded pending student kept "Teacher accounts" stuck at 1 after every approval, and "5 active users overall" under "Students 4" counted admins and teachers | ✅ `UserStats.by_role_status` cross-tab from one `GROUP BY role, status`; `by_role`/`by_status` now derived from it, so the three can't disagree |
