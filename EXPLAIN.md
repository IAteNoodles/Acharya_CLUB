...
- **Rotation**: `POST /auth/refresh` issues a new pair and **blacklists the old refresh
  token**. Refresh is denied (403) if the account is no longer ACTIVE. `POST /auth/logout`
  blacklists the refresh token. `POST /auth/change-password` (authenticated) verifies the
  current password before rehashing.
...
**Signup rules**: email must end with `@college.edu` and is trimmed + lowercased before
storage and lookup (emails are case-insensitive; login lowercases too); students become
ACTIVE immediately; teachers are created PENDING and must be approved by an admin
(`/users/{id}/approve`); admins exist only via seeding. Login failures return one generic
...
- **Events** (`services/event.py`): visibility is role-scoped — students see only APPROVED,
  ...
  PENDING). `out_college` events are participation-only — category must be `participant`
  at creation and can't change away from it. Dates validated (no past start, end ≥ start).
  Approve/reject allowed for admin or the assigned coordinator (who must have role teacher);
  can't approve a REJECTED event, re-approve, or reject an APPROVED event. Creators may edit
  only DRAFT/PENDING events (admins any). Approval/rejection notifies the creator.
- **Registrations** (`services/registration.py`): only for APPROVED events **with a
  coordinator assigned**; `role_type` must match the event category (`both` accepts either);
  capacity = `max_registrations` counted against ACCEPTED rows only (0 = unlimited), enforced
  at registration *and* re-checked at accept; a student registers at most **once per event**
  (dual-role registration is rejected — the DB constraint is still per (event, student,
  role_type), the service enforces the stricter rule). Accept/reject only from PENDING, only
  by admin or the coordinator; both notify the student.
- **Attendance** (`services/attendance.py`): bulk upsert of 1–100 records per call, only by
  admin/coordinator, only for APPROVED events, only for dates within the event's start/end
  window, only for students holding an ACCEPTED registration; unique per
